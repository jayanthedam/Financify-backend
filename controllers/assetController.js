...
const fetchStockPrices = async (req, res) => {
  try {
    ...
    for (const asset of assets) {
      ...
      const currentValue = currentPrice * asset.details.shares;
      const investedValue = asset.details.pricePerShare * asset.details.shares;
      
      totalValue += currentValue;
      stockValues[asset.details.ticker] = {
        currentPrice,
        currentValue,
        investedValue,
        shares: asset.details.shares,
        pnl: currentValue - investedValue,
        pnlPercentage: investedValue ? ((currentValue - investedValue) / investedValue) * 100 : 0 // Ensure no division by zero
      };
    }
...
const fetchGoldPrice = async (req, res) => {
  try {
    const goldPriceData = await GoldPriceModel.findOne().sort({ createdAt: -1 });
    if (!goldPriceData) {
      return res.status(404).json({ message: 'Gold price data not found' });
    }
    res.json({ goldPrice: goldPriceData.price_gram_24k });
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
};
...
export { fetchStockPrices, fetchGoldPrice, createAsset, getAssets, updateAsset, deleteAsset };
// import fetch from 'node-fetch';
// import Asset from '../models/AssetModel.js';
